...
void mergeSortParellel(int arr[] ,int left,int right){
    ...
    if(left<right){
        ...
        {
        { mergeSortParellel(arr,left,mid);}
        #pragma omp section
        { mergeSortParellel(arr,mid+1,right);}
    }
    merge(arr,left,mid,right);
}
}
int main(){
    ...
    mergeSortParellel(arr_par,0,n-1);
    double time_par=omp_get_wtime()-start_time;
    printf("Parallel merge sort : %f seconds\n",time_par);
    printf("difference (sequential - parallel) :%f seconds\n",time_seq-time_par);
    if(time_par>0) printf("speedup: %.2fx\n",time_seq/time_par);
    printf("\nverification first 20  ele sorted array\n");
